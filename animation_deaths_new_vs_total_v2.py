...
def thread_for_plotting(raw_data, start_point, end_point, counter_i, thread_num):
    ...
    for date in range(start_point, end_point + 1):
        ...
        for c in countries:
            df = pd.DataFrame(data=new_raw_data[c][new_raw_data[c] >= 1])
            df[c + '_new'] = np.nan
        ...
        plt.title('New Deaths vs Total Deaths on ' + as_of_date)
        ...
        plt.savefig('animation/' + str(counter_i).zfill(4) + '_deaths_new_vs_total.png', dpi=200)
        plt.close()
