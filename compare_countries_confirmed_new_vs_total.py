...
# Un/comment line below if you need to output grouped data into CSV
...
plt.grid()
for c in countries:
    df = pd.DataFrame(data=raw_data[c][raw_data[c] > 100])
    df[c+'_new'] = np.nan
...
plt.savefig('graphs/confirmed_new_vs_total.png', dpi=200)
